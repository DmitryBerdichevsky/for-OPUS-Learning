...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЭлементОтборПоРесурсу <> Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(ЭлементОтборПоРесурсу.Значение) Тогда
			...
			ТекстСообщения = НСтр("ru = 'Не указан согласуемый ресурс, для которого формируется отчет.'");
		...
		Если Не ЭлементОтборПоРесурсу.Использование Тогда
			...
			ТекстСообщения = НСтр("ru = 'Небходимо включить использование отбора по согласуемому ресурса, для которого формируется отчет.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
		КонецЕсли;
