...
Процедура ЗаполнитьРеквизитыВетки(Ветка, РазрабатываемыйОбъект=Неопределено, ДанныеЗаполнения=Неопределено) Экспорт
	...
	Ветка.ПриоритетТестирования = Перечисления.ПриоритетыТестированияВеток.Средний;
...
Функция ИмяВеткиРепозиторияПоРазрабатываемомуОбъекту(РазрабатываемыйОбъект, Ответственный=Неопределено, 
	...
	Если ТипРазрабатываемогоОбъекта = Тип("СправочникСсылка.ТехническиеПроекты") Тогда
		...
		Если ЗначениеЗаполнено(РазрабатываемыйОбъект.Код) Тогда
			МассивСтрок.Добавить(РазрабатываемыйОбъект.Код);
			МассивСтрок.Добавить("-");
		КонецЕсли;
		СтрокаОтветственный = ИмяПользователяДляИмениВетки(РазрабатываемыйОбъект.Ответственный);
		Если ЗначениеЗаполнено(СтрокаОтветственный) Тогда
			МассивСтрок.Добавить(СтрокаОтветственный);
			МассивСтрок.Добавить("-");
		КонецЕсли;
		Наименование = СтрЗаменить(Строка(РазрабатываемыйОбъект.Наименование), " ", "-");
	...
	ИначеЕсли ТипРазрабатываемогоОбъекта = Тип("СправочникСсылка.ВерсииПроекта") Тогда
		МассивСтрок = Новый Массив();
		МассивСтрок.Добавить(Строка(РазрабатываемыйОбъект.НомерРедакции));
		МассивСтрок.Добавить(Строка(РазрабатываемыйОбъект.НомерПодредакции));
		МассивСтрок.Добавить(Строка(РазрабатываемыйОбъект.НомерВерсии));
	...
	ИначеЕсли ТипРазрабатываемогоОбъекта = Тип("СправочникСсылка.Ошибки") 
		...
		ВеткаОбнаружения = Источник;
		Если ТипРазрабатываемогоОбъекта = Тип("СправочникСсылка.Ошибки") Тогда
			ВеткаОбнаружения = ?(ЗначениеЗаполнено(Источник), Источник, РазрабатываемыйОбъект.ВеткаОбнаружения);
			Если ЗначениеЗаполнено(ВеткаОбнаружения.Имя) Тогда
				МассивСтрок.Добавить(ВеткаОбнаружения.Имя);
			ИначеЕсли ЗначениеЗаполнено(РазрабатываемыйОбъект.СборкаОбнаружения) Тогда
				МассивСтрок.Добавить(РазрабатываемыйОбъект.СборкаОбнаружения);
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(ВеткаОбнаружения.Имя) Тогда
			МассивСтрок.Добавить(ВеткаОбнаружения.Имя);
		КонецЕсли;
			
		МассивСтрок.Добавить(ИмяПользователяДляИмениВетки(Ответственный));
		...
		Если  ТипРазрабатываемогоОбъекта = Тип("СправочникСсылка.Ошибки") Тогда
			СуществующиеВетки = ВеткиСДублирующимсяИменем(ИмяВетки, РазрабатываемыйОбъект.Владелец);
	...
	ИначеЕсли ТипРазрабатываемогоОбъекта = Тип("СправочникСсылка.Ветки") 
		...
		МассивСтрок = Новый Массив();
		МассивСтрок.Добавить("bugfix");
		МассивСтрок.Добавить(РазрабатываемыйОбъект.Имя);
		МассивСтрок.Добавить(ИмяПользователяДляИмениВетки(Ответственный));
		ДатаСтрокой = Формат(ДатаСоздания, "ДЛФ=D");
		МассивСтрок.Добавить(ДатаСтрокой);
		ИмяВетки = СтрСоединить(МассивСтрок, "-");
...
Функция НаименованиеВеткиРепозиторияПоРазрабатываемомуОбъекту(РазрабатываемыйОбъект, Знач ИмяВетки = "", Ответственный=Неопределено, 
	...
	СтрокаОтветственный = ИмяПользователяДляИмениВетки(Ответственный);
...
Функция ПроверкаИсправленияОшибокВВетке(Ветка, ПринудительнаяПроверка=Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Выборка.ОшибкиИзВеткиСсылка) И НЕ Выборка.ОшибкаИсправленаВДругойВеткеСОдинаковымИменем Тогда
...
Функция ИмяПользователяДляИмениВетки(Пользователь)
