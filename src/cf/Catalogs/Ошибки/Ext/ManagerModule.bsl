...
//
// Параметры:
//  Ошибка - Тип - СправочникОбъект.Ошибка, СправочникСсылка.Ошибки - ошибка, описание которой требуется получить
//
// Возвращаемое значение:
//  Описание - ТИП - Строка. Строка, содержащая html-описание ошибки
//
Функция ПолнаяИнформацияПоОшибке(Ошибка, НовыйСрокОтработки) Экспорт
	...
	Если ОшибкаОбъект.МетодВыявленияОшибки = Перечисления.МетодыВыявленияОшибок.Автотесты И 
		...
		КоличествоЗаблокированныхТестов = КоличествоЗаблокированныхОшибок(ОшибкаОбъект);
	...
	Если Ошибка.Обнаружена = Перечисления.МестаОбнаруженияОшибок.ВВетке Тогда
		ТипВетки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ошибка.ВеткаОбнаружения, "Тип");
	Иначе
	...
	Если ТипВетки = Перечисления.ТипыВеток.ВеткаВерсии
		...
		ИЛИ Ошибка.Обнаружена = Перечисления.МестаОбнаруженияОшибок.ВСборке Тогда
	...
	ИсправленаБезДетализацииИЛИНеИсправлена = Не ОшибкаОбъект.Исправлена 
	                                      ИЛИ (ОшибкаОбъект.Исправлена 
	                                           И ОшибкаОбъект.ТипПроблемы = Перечисления.ТипПроблемы.БезДетализации);
	
	Если ОшибкаОбъект.СписокОбъектов.Количество() > 0 И ИсправленаБезДетализацииИЛИНеИсправлена Тогда
	...
	Если ЗначениеЗаполнено(ОшибкаОбъект.СпособОбхода)
		...
		ИЛИ ОшибкаОбъект.ИзмененныеМетаданные.Количество()>0
	...
	Если ОшибкаОбъект.ИзмененныеМетаданные.Количество()>0
		И ОшибкаОбъект.Исправлена Тогда
		ВывестиТаблицуОбъектыМетаданных(ОшибкаОбъект, Макет, ТекстовыйДокумент);
		ДобавитьАбзац(ТекстовыйДокумент);
	КонецЕсли;
...
//
Функция ВерсияИсправления(Проект, Обнаружена, ВеткаОбнаружения, СборкаОбнаружения, СрочностьИсправления, ИнформацияИзВнешнихИсточников, ВариантИсправления) Экспорт
	...
	Если Обнаружена = Перечисления.МестаОбнаруженияОшибок.ВВетке Тогда
		ВерсияОбнаружения = ВерсияОбнаруженияДляВетки(ВеткаОбнаружения);
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПроектыПорядокВоспроизведенияИИсправленияОшибок." + КолонкаВерсииИсправления + " КАК ВерсияИсправления
	|ИЗ
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПроектыПорядокВоспроизведенияИИсправленияОшибок.НомерСтроки"
	;
...
Функция ВариантИсправления(ВеткаОбнаружения, МетодВыявления, ВерсияИсправления) Экспорт
	...
	Иначе
		...
		Если ДанныеВеткиВерсииИсправления.Тип = Перечисления.ТипыВеток.ОсновнаяВеткаПроекта Тогда
...
Процедура НазначитьВерсиюИсправления(МассивОшибок, ВерсияИсправления, КоличествоОбработанных) Экспорт
	...
	РеквизитыВерсии = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВерсияИсправления, "Владелец,Ветка");
	
	ПроектВерсии = РеквизитыВерсии.Владелец;
	Ветка = РеквизитыВерсии.Ветка;
...
Функция ДанныеОЗамещенииПользователяВРаботеСОшибками(Пользователь, Проект) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.Проект) Тогда
			...
			// чем общие (без указания проетка).
			Прервать;
		КонецЕсли;
...
Функция ТребуетсяПересчетСрокаОтработки(СтарыеЗначенияРеквизитов, ТекущиеЗначенияРеквизитов, ТребуетсяОтработкаПоручения=Ложь, ИзменяетсяДатаНаправления=Ложь) Экспорт
	...
	Если СтарыеЗначенияРеквизитов.ВерсияИсправления <> ТекущиеЗначенияРеквизитов.ВерсияИсправления
		...
		И ТекущиеЗначенияРеквизитов.Исправляется = Перечисления.ВариантыИсправленияОшибок.ВРазныхВетках Тогда
		ИзменилисьУсловияРасчета = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьИзмененныеМетаданныеиОбработчики(ОшибкаОбъект) Экспорт
	...
	Для Каждого Строка Из ОшибкаОбъект.СписокОбъектов Цикл
		...
		Если ОшибкаОбъект.ТипПроблемы = Перечисления.ТипПроблемы.ОбъектыМетаданных Тогда
		
			Отбор = Новый Структура("ОбъектМетаданных,ПодчиненныйОбъект,УточнениеРасположения,Комментарий");
			Отбор.ОбъектМетаданных      = Строка.ПроблемныйОбъект;
			Отбор.ПодчиненныйОбъект     = Строка.ПодчиненныйОбъект;
			Отбор.УточнениеРасположения = Строка.Уточнение;
			Отбор.Комментарий           = Строка.Комментарий;
			
			Если ОшибкаОбъект.ИзмененныеМетаданные.НайтиСтроки(Отбор).Количество() = 0 Тогда
			
				НоваяСтрока = ОшибкаОбъект.ИзмененныеМетаданные.Добавить();
				НоваяСтрока.ОбъектМетаданных      = Строка.ПроблемныйОбъект;
				НоваяСтрока.ПодчиненныйОбъект     = Строка.ПодчиненныйОбъект;
				НоваяСтрока.УточнениеРасположения = Строка.Уточнение;
				НоваяСтрока.Комментарий           = Строка.Комментарий;
				
				Если ТипЗнч(Строка) = Тип("ДанныеФормыЭлементКоллекции") И Строка.Свойство("ПодчиненныйОбъектСтрока") Тогда
					НоваяСтрока.ПодчиненныйОбъектСтрока = Строка.ПодчиненныйОбъектСтрока;
				КонецЕсли; 
				
			КонецЕсли; 
		
		ИначеЕсли ОшибкаОбъект.ТипПроблемы = Перечисления.ТипПроблемы.ОбработчикиОбновления Тогда
...
//
Функция ВерсияОбнаруженияДляВетки(Ветка) Экспорт
	...
	Если ТипВетки = Перечисления.ТипыВеток.ВеткаТехническогоПроекта Тогда
		Версия = ВерсияОбнаруженияДляВеткиТехническогоПроекта(Ветка);
	...
	ИначеЕсли ТипВетки = Перечисления.ТипыВеток.ОсновнаяВеткаПроекта Тогда
		Версия = ВерсияОбнаруженияДляВеткиПроекта(Ветка);
	ИначеЕсли ТипВетки = Перечисления.ТипыВеток.ВеткаДляИсправленияОшибок Тогда
		Версия = ВерсияОбнаруженияДляВеткиИсправленияОшибок(Ветка);
	КонецЕсли;
...
//
Функция ВеткиДляИсправленияОшибки(Проект, ВерсияИсправления) Экспорт
	...
	|ИЗ
	|	Справочник.ВерсииПроекта КАК ВерсииПроекта
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТВерсияИсправления КАК ВТВерсияИсправления
	|		ПО ВерсииПроекта.ПорядковыйНомер >= ВТВерсияИсправления.ПорядковыйНомер
	|ГДЕ
	|	ВерсииПроекта.Владелец = &Проект
	|	И НЕ ВерсииПроекта.ПометкаУдаления
	|	И ВерсииПроекта.Ветка <> ЗНАЧЕНИЕ(Справочник.Ветки.ПустаяСсылка)
	|	И (ВерсииПроекта.ДатаПрекращенияПоддержки >= &ТекущаяДата
	|			ИЛИ ВерсииПроекта.ДатаПрекращенияПоддержки = ДАТАВРЕМЯ(1, 1, 1)
	|			ИЛИ ВерсииПроекта.Ссылка = &ВерсияИсправления)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
...
Процедура ВывестиТаблицуОбъектыМетаданных(ОшибкаОбъект, Макет, ТекстовыйДокумент)
	
	ПараметрыТекста = Новый Структура("РазмерШрифта", 2);
	ОбъектыМетаданных = ОшибкаОбъект.ИзмененныеМетаданные;
	ТекстовыйДокумент.ДобавитьСтроку("<TABLE border=1><TBODY><TR Align=middle>");
	
	ЕстьПодчиненныеОбъекты = ОбъектыМетаданных.НайтиСтроки(Новый Структура("ПодчиненныйОбъект", Неопределено)).Количество()
			<> ОбъектыМетаданных.Количество();
			
	ТекстовыйДокумент.ДобавитьСтроку("<TD>");
	ТекстПоля = НСтр("ru='Объект метаданных'");
	...
	ТекстовыйДокумент.ДобавитьСтроку("<TD>");
	ТекстПоля = НСтр("ru='Уточнение расположения'");
	ВывестиПоле(ТекстовыйДокумент, Макет, ТекстПоля, "", Истина, , ПараметрыТекста);
	ТекстовыйДокумент.ДобавитьСтроку("</TD>");
	...
	Для Каждого Строка Из ОбъектыМетаданных Цикл
		ТекстовыйДокумент.ДобавитьСтроку("<TR>");
		ТекстовыйДокумент.ДобавитьСтроку("<TD valign=""top"">");
		СтрокаСсылки = "#" + ПолучитьНавигационнуюСсылку(Строка.ОбъектМетаданных) + "#";
		ПредставлениеСсылки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Строка.ОбъектМетаданных, "Имя");
		...
		ТекстовыйДокумент.ДобавитьСтроку("<TD valign=""top"">");
		ТекстПоля = Строка.УточнениеРасположения;
		ВывестиПоле(ТекстовыйДокумент, Макет, ТекстПоля, "", Истина, , ПараметрыТекста);
		ТекстовыйДокумент.ДобавитьСтроку("</TD>");
		
		ТекстовыйДокумент.ДобавитьСтроку("<TD valign=""top"">");
		ТекстПоля = Строка.Комментарий;
		ВывестиПоле(ТекстовыйДокумент, Макет, ТекстПоля, "", Истина, , ПараметрыТекста);
		ТекстовыйДокумент.ДобавитьСтроку("</TD>");
		ТекстовыйДокумент.ДобавитьСтроку("</TR>");
	КонецЦикла;
...
Функция КоличествоЗаблокированныхОшибок(ОшибкаОбъект, Новые=Ложь)
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Ошибки.Ссылка КАК Ссылка
		...
		|	РезультатыРегистрацииОшибокСрезПоследних.ЭлементТеста КАК ЭлементТеста,
		|	РезультатыРегистрацииОшибокСрезПоследних.ЗапускТестирования КАК ЗапускТестирования
		|ПОМЕСТИТЬ ТестыДляОтбора
		|ИЗ
		|	РегистрСведений.РезультатыРегистрацииОшибок.СрезПоследних КАК РезультатыРегистрацииОшибокСрезПоследних
		|ГДЕ
		|	РезультатыРегистрацииОшибокСрезПоследних.Ошибка В
		|			(ВЫБРАТЬ
		|				ОшибкиДляОтбора.Ссылка
		|			ИЗ
		|				ОшибкиДляОтбора КАК ОшибкиДляОтбора)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Ветка,
		|	Тест,
		...
		|	РезультатыВыполненияТестовСрезПоследних.ЭлементТеста КАК ЭлементТеста,
		...
		|			,
		|			(Ветка, Тест, ЭлементТеста) В
		|				(ВЫБРАТЬ
		|					ТестыДляОтбора.Ветка,
		|					ТестыДляОтбора.Тест,
		|					ТестыДляОтбора.ЭлементТеста
		|				ИЗ
		|					ТестыДляОтбора)) КАК РезультатыВыполненияТестовСрезПоследних
		|ГДЕ
		|	РезультатыВыполненияТестовСрезПоследних.Статус = &Статус
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СрезПоследних.Тест КАК Тест,
		|	СрезПоследних.ЭлементТеста КАК ЭлементТеста
		|ИЗ
		...
		|			И СрезПоследних.ЭлементТеста = ТестыДляОтбора.ЭлементТеста
		|			И СрезПоследних.ЗапускТестирования = ТестыДляОтбора.ЗапускТестирования
		|ГДЕ
		|	СрезПоследних.Статус = &Статус";

	Запрос.УстановитьПараметр("Ошибка", ОшибкаОбъект.Ссылка);
	Запрос.УстановитьПараметр("Статус", Перечисления.СтатусыВыполненияТеста.ВыполненСОшибкой);
...
Функция ЗарегистрироватьСУчетомДублей(ТаблицаРегистрации, МестоВозникновения, ПараметрыОшибки = Неопределено, ПоляГруппировки = "")
...
Процедура ПоискДублирующихОшибок(Таблица, МестоВозникновения, ПараметрыОшибки = Неопределено)
...
Функция ВерсияОбнаруженияДляВеткиТехническогоПроекта(Ветка)
...
Функция ВерсияОбнаруженияДляВеткиПроекта(Ветка)
...
Функция ВерсияОбнаруженияДляВеткиИсправленияОшибок(Ветка)
	...
	Если ДанныеВетки.ПриемникТип <> Перечисления.ТипыВеток.ВеткаДляИсправленияОшибок Тогда
		Возврат ВерсияОбнаруженияДляВетки(ДанныеВетки.Приемник);
	КонецЕсли;
...
//
Функция ВерсияОбнаруженияДляВеткиВерсии(Ветка)
...
Процедура ЗаписатьОшибкуВРегистрРезультатыРегистрацииОшибок(Ошибка, СсылкаНаОшибку, ЭтоДубль, ПараметрыРегистрации)
	...
	ЗапускТестирования = ЗапускТестирования(ИдЗапуска);
	Если НЕ ЗначениеЗаполнено(ЗапускТестирования) Тогда
		...
		НовыйЗапускТестирования.Проект = Ошибка.Владелец;
		НовыйЗапускТестирования.Ветка = Ветка; 
		НовыйЗапускТестирования.ВеткаСтрокой = Ветка.Имя;
		НовыйЗапускТестирования.Дата = ТекущаяДата();
		НовыйЗапускТестирования.Проведен = Истина;
		НовыйЗапускТестирования.Записать(РежимЗаписиДокумента.Запись);
		ЗапускТестирования = НовыйЗапускТестирования.Ссылка;
	КонецЕсли;
		
	ИсходнаяИнформация = Ошибка.ИсходнаяИнформацияОбОшибке + Символы.ПС + Символы.ПС + Ошибка.ПорядокВоспроизведения;

	ЭлементыТеста = Новый Массив;
	Если Ошибка.Тест.РезультатыТестаДетализируютсяПоОбъектамМетаданных Тогда 
...
Функция ЗапускТестирования(ИдентификаторЗапуска)
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
...
Функция ВерсииИСборкиИсправленияОшибки(ОшибкаОбъект)
	...
	|	ВеткиДляИсправления.ТипВетки КАК ТипВетки,
	|	ВеткиДляИсправления.ДатаОбнаружения КАК ДатаОбнаружения
	|ПОМЕСТИТЬ ВТСборки
	|ИЗ
