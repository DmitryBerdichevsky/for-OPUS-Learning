...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ИспользоватьРегламентноеЗаданиеСообщенийПоОшибкам Тогда
		НепроверяемыеРеквизиты.Добавить("ПользовательАвторСообщенийПоОшибкам");
	КонецЕсли;
...
Процедура ПроверитьДублиВерсииВоспроизведения(Отказ)
	...
	СтрокаТЧ = ПорядокВоспроизведенияИИсправленияОшибок.Найти(Справочники.ВерсииПроекта.ПустаяСсылка(), "ВерсияВоспроизведения");
	Если СтрокаТЧ <> Неопределено Тогда
		Возврат;
	КонецЕсли;
