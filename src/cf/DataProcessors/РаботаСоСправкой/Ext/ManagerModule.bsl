...
Функция ДеревоМетаданныхДляСправки(Проект, Отметка = Ложь) Экспорт
	...
	ДеревоМетаданных.Колонки.Добавить("ТехническийПроектИсточник");
		...
		Если СтрокаОбъекта.Отметка = 1 Тогда
			...
			Если   ЗначениеЗаполнено(СтрокаОбъекта.ОбъектСправки)
				...
				Если Не ВыгружатьВФорматеEDT Тогда
					...
					Если Не ЗначениеЗаполнено(СтрокаОбъекта.ТехническийПроектИсточник) Тогда
...
Функция ТекстЗапросаДляФормированияДерева()
	...
	|	Подсистемы.ТехническийПроектИсточник,
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ВключатьВСправку,
	|	ОбъектыМетаданных.ТехническийПроектИсточник,
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ВключатьВСправку,
	|	ФормыОбъектовМетаданных.ТехническийПроектИсточник,
...
Функция ТекстОписанияСправки(СтрокаДерева, ИмяФайлаСправки)
	...
	ЗаписьXML.ЗаписатьАтрибут("version", "2.8");
