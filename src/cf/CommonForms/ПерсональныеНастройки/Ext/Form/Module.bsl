...
Процедура НастроитьПараметрыАвторизацииВ1СДокументооборот(Команда)
	...
	ОткрытьФорму("Обработка.ИнтеграцияС1СДокументооборот.Форма.АвторизацияВ1СДокументооборот",,,,,, Неопределено);
...
Процедура ЗаполнитьТаблицуНастроекУведомлений()
	...
	|	ЕСТЬNULL(НастройкиУведомленийПользователей.УведомлятьОХодеВыполненияТехническихПроектов, ЛОЖЬ) КАК УведомлятьОХодеВыполненияТехническихПроектов,
