...
Функция СформироватьЗапросУзловДляОбъекта(ТипЗначенияОбъекта = "") Экспорт
	...
	ИначеЕсли ТипЗначенияОбъекта = "СправочникОбъект.ТехническиеПроекты" Тогда
		...
		|";
		Возврат ТекстЗапроса;
	ИначеЕсли ТипЗначенияОбъекта = "СправочникОбъект.КонтрольныеТочкиТехническихПроектов"
		Или ТипЗначенияОбъекта = "ПланВидовХарактеристикОбъект.ВопросыПоТехническимПроектам" Тогда
		ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	РИБСОтборамиПроекты.Ссылка
		|ИЗ
		|	ПланОбмена.РИБСОтборами.Проекты КАК РИБСОтборамиПроекты
	...
	ИначеЕсли ТипЗначенияОбъекта = "РегистрСведенийЗапись.РезультатыОтветовНаВопросыТехническихПроектов"
		Или ТипЗначенияОбъекта = "РегистрСведенийЗапись.СогласованиеТехническихПроектов" Тогда
		...
		|	И РИБСОтборамиПроекты.Ссылка.ЭтотУзел = ЛОЖЬ
		|	И РИБСОтборамиПроекты.Ссылка.ПометкаУдаления = ЛОЖЬ
		|	И РИБСОтборамиПроекты.Проект = ТехническиеПроекты.Владелец
		|	И НЕ РИБСОтборамиПроекты.Ссылка.ИспользоватьОтборПоРазделамПроектов
		|	И РИБСОтборамиПроекты.СинхронизироватьТехническиеПроекты
		|	И (НЕ РИБСОтборамиПроекты.ТолькоПубликуемыеТехническиеПроекты
		|		ИЛИ &СвойствоОбъекта_Публикуется)
		...
		|ПО РИБСОтборамиПроекты.Ссылка = РИБСОтборамиРазделыПроектов.Ссылка
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	Справочник.ТехническиеПроекты КАК ТехническиеПроектыШапка
		|ПО РИБСОтборамиРазделыПроектов.РазделПроекта = ТехническиеПроектыШапка.РазделПроекта
		|	И ТехническиеПроектыШапка.Ссылка = &СвойствоОбъекта_Ссылка
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	Справочник.ТехническиеПроекты.РазделыПроекта КАК ТехническиеПроектыРазделы
		|ПО РИБСОтборамиРазделыПроектов.РазделПроекта = ТехническиеПроектыРазделы.Раздел
		|	И ТехническиеПроектыРазделы.Ссылка = &СвойствоОбъекта_Ссылка
		|ГДЕ
		|	РИБСОтборамиПроекты.Ссылка.ЭтотУзел = ЛОЖЬ
		|	И РИБСОтборамиПроекты.Ссылка.ПометкаУдаления = ЛОЖЬ
		|	И РИБСОтборамиРазделыПроектов.Ссылка.ИспользоватьОтборПоРазделамПроектов
		|	И (ТехническиеПроектыШапка.Ссылка IS NOT NULL
		|		ИЛИ ТехническиеПроектыРазделы.Ссылка IS NOT NULL)
		|	И РИБСОтборамиПроекты.СинхронизироватьТехническиеПроекты
		|	И (НЕ РИБСОтборамиПроекты.ТолькоПубликуемыеТехническиеПроекты
		|		ИЛИ &СвойствоОбъекта_Публикуется)
		|";
	...
	ИначеЕсли ТипЗначенияОбъекта = "ДокументОбъект.УчетВремени" Тогда
		...
		|	РИБСОтборамиПроекты.Ссылка.ЭтотУзел = ЛОЖЬ
		|	И РИБСОтборамиПроекты.Ссылка.ПометкаУдаления = ЛОЖЬ
		|	И РИБСОтборамиПроекты.СинхронизироватьДанныеУчетаВремени
...
Функция ПроверятьПринадлежностьДопсвойстваПроекту(НаборСвойствСсылка) Экспорт
	...
	МассивИсключенийНаборов.Добавить(Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_Задачи_Общие);
	...
	МассивНаборовСвязанныхСПроектами.Добавить(Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_Задачи);
