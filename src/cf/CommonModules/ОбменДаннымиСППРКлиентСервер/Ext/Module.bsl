...
Функция НастройкаОтборовНаУзле() Экспорт
	...
	СтруктураНастроек.Вставить("СинхронизироватьДополнительныеОтчетыИОбработки", Ложь);
	...
	СтруктураТабличнойЧастиПроекты.Вставить("СинхронизироватьДанныеУчетаВремени", Новый Массив);
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле) Экспорт
	...
	МассивПроектовДанныеУчВремени = Новый Массив;
	...
	Для Каждого ТекПроект Из НастройкаОтборовНаУзле.Проекты.Проект Цикл
		...
		Если НастройкаОтборовПроекты.СинхронизироватьРегламенты[Индекс] Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Если НастройкаОтборовПроекты.СинхронизироватьДанныеУчетаВремени[Индекс] Тогда
			МассивПроектовДанныеУчВремени.Добавить(ТекПроект);
		КонецЕсли;
	...
	ДополнитьОписаниеДляДанных(НСтр("ru='данные учета времени'"), МассивПроектовДанныеУчВремени, ТекстОписания);
