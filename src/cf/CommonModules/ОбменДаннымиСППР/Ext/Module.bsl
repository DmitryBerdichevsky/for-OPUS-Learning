...
Функция ПолучитьУзлыДляОбъекта(ЭлементДанных,ТипЗначенияОбъекта = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗначенияОбъекта = Тип("СправочникОбъект.Техническиепроекты") Тогда
		...
		Запрос.УстановитьПараметр("СвойствоОбъекта_МассивРазделов",ОбменДаннымиПовтИспСППР.МассивРазделовПроектаИзСсылки(ЭлементДанных.Ссылка));
		Запрос.УстановитьПараметр("СвойствоОбъекта_Публикуется",СтатусПубликацииДляОбъекта(ЭлементДанных.Ссылка));
	ИначеЕсли ТипЗначенияОбъекта = Тип("СправочникОбъект.КонтрольныеТочкиТехническихПроектов") Тогда
		ТекстЗапроса = ОбменДаннымиПовтИспСППР.СформироватьЗапросУзловДляОбъекта(СтроковоеПредставлениеТипа);
		Запрос = Новый Запрос(ТекстЗапроса);
		Запрос.УстановитьПараметр("СвойствоОбъекта_Ссылка",ЭлементДанных.Владелец);
	...
	ИначеЕсли ТипЗначенияОбъекта = Тип("ПланВидовХарактеристикОбъект.ВопросыПоТехническимПроектам") Тогда
		
		ТекстЗапроса = ОбменДаннымиПовтИспСППР.СформироватьЗапросУзловДляОбъекта(СтроковоеПредставлениеТипа);
		Запрос = Новый Запрос(ТекстЗапроса);
		Запрос.УстановитьПараметр("СвойствоОбъекта_Ссылка",ЭлементДанных.Проект);
		Запрос.УстановитьПараметр("СвойствоОбъекта_Публикуется",СтатусПубликацииДляОбъекта(ЭлементДанных.Проект));
		
	ИначеЕсли ТипЗначенияОбъекта = Тип("РегистрСведенийЗапись.РезультатыОтветовНаВопросыТехническихПроектов")
		Или ТипЗначенияОбъекта = Тип("РегистрСведенийЗапись.СогласованиеТехническихПроектов") Тогда
		
		ТекстЗапроса = ОбменДаннымиПовтИспСППР.СформироватьЗапросУзловДляОбъекта(СтроковоеПредставлениеТипа);
		Запрос = Новый Запрос(ТекстЗапроса);
		Запрос.УстановитьПараметр("СвойствоОбъекта_Ссылка",ЭлементДанных.ТехническийПроект);
		Запрос.УстановитьПараметр("СвойствоОбъекта_Публикуется",СтатусПубликацииДляОбъекта(ЭлементДанных.ТехническийПроект));
