...
Функция ПолучитьФайлыИзGIT(Параметры) Экспорт
	...
	Если Параметры.Свойство("ТехническийПроектИсточник") и ЗначениеЗаполнено(Параметры.ТехническийПроектИсточник) Тогда
		СоздатьВремКаталог = Истина;
		РеквизитыТехПроекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Параметры.ТехническийПроектИсточник, "Версия, Ветка.Имя, Ветка.Источник.Имя");
		ИмяВеткиРепозитория = РеквизитыТехПроекта.ВеткаИмя;
		ИмяВеткиисточника   = РеквизитыТехПроекта.ВеткаИсточникИмя;
	КонецЕсли;
	...
	Если Параметры.Свойство("ТехническийПроектИсточник") 
		и ЗначениеЗаполнено(Параметры.ТехническийПроектИсточник) Тогда
		
		ФайлЛогаТекстДокумент.Очистить();
		
		РеквизитыВерсииПроекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитыТехПроекта.Версия, "Ветка.Имя");
		ИмяВеткиРепозиторияБазовая = "origin/" + ?(ЗначениеЗаполнено(ИмяВеткиИсточника),
											ИмяВеткиИсточника,РеквизитыПроекта.ВеткаИмя);
		...
		ТекстКоманды = СтрЗаменить(ТекстКоманды, "%BRANCH%", РеквизитыТехПроекта.ВеткаИмя);
...
Функция ВыполнитьКонсольнуюКоманду(ТекстКоманды, КаталогКоманды, ИсключениеПриОшибке = Истина) Экспорт
	...
	ТекстКонсольнойКоманды = СтрШаблон("%1 >> ""%2""", ТекстКоманды, ФайлЛога);
