...
Функция ЗарегистрироватьОшибку(ПараметрыОшибки, ОбъектыМетаданных) Экспорт
	...
	СрочностьИсправления = ?(ЗначениеЗаполнено(ПараметрыОшибки.СрочностьИсправления),
	                         Перечисления.СрочностьИсправленияОшибок[ПараметрыОшибки.СрочностьИсправления],
	                         Перечисления.СрочностьИсправленияОшибок.ВПлановомПорядке);
	...
	Если ОбъектыМетаданных <> Неопределено Тогда
		...
		Для Каждого СтрокаОбъектаМетаданных Из ОбъектыМетаданных Цикл
			...
			Если Не РазделУказан И ЗначениеЗаполнено(СтрокаОбъектаМетаданных.РазделПроекта) Тогда
				СтрокаРазделОтветственныйОбъектовМетаданных.РазделПроекта = СтрокаОбъектаМетаданных.РазделПроекта;
			КонецЕсли;
			
			Если Не КомуНаправленаУказан И ЗначениеЗаполнено(СтрокаОбъектаМетаданных.Ответственный) Тогда
	...
	ФорматированнаяСтрока   = Новый ФорматированнаяСтрока(ПараметрыОшибки.ИсходнаяИнформация);
	ФорматированныйДокумент = Новый ФорматированныйДокумент;
...
Функция ЗарегистрироватьОшибкуПоФайлуXML(Файл, Настройки) Экспорт 
	...
	Возврат ЗарегистрироватьОшибкуПоДаннымСтруктуры(СтруктураОшибки, Настройки);
	
КонецФункции

Функция ЗарегистрироватьОшибкуПоДаннымСтруктуры(СтруктураОшибки, Настройки) Экспорт 
	...
	Для Каждого Строка Из СтруктураОшибки.ОбъектыМетаданных Цикл
	...
	Для Каждого ИмяФайла Из СтруктураОшибки.ИменаФайлов Цикл
		...
		Если СтрЗаканчиваетсяНа(ИмяФайлаНРег, ".1cd") ИЛИ СтрЗаканчиваетсяНа(ИмяФайлаНРег, ".dt") ИЛИ СтрЗаканчиваетсяНа(ИмяФайлаНРег, ".cf") Тогда
	...
	Если Не Ответ.ЭтоДублирующаяОшибка Тогда
		...
		Для Каждого ИмяФайла Из СтруктураОшибки.ИменаФайлов Цикл
			...
			ИмяФайлаНРег = НРег(ИмяФайла);
			ЭтоФайлБазы  = СтрЗаканчиваетсяНа(ИмяФайлаНРег, ".1cd") ИЛИ СтрЗаканчиваетсяНа(ИмяФайлаНРег, ".dt") ИЛИ СтрЗаканчиваетсяНа(ИмяФайлаНРег, ".cf");
...
Функция ПодобратьДанныеВерсииПроекта(Проект, НомерВерсии)
...
Функция ПодобратьДанныеВариантаОбнаружения(ПараметрыОшибки, Проект)
	...
	Ответ = Новый Структура("Обнаружена, СценарийПользователя, ОтветственныйСценария, НастройкаЗапускаСценария, 
	                        ...
	                        |ДатаОбнаружения, ДатаНаследования, ВеткаОбнаружения, Тест, ОтветственныйТеста");
	...
	Если ЗначениеЗаполнено(Ответ.НастройкаЗапускаПроцесса) Тогда
	...
	ИначеЕсли ЗначениеЗаполнено(Ответ.НастройкаЗапускаСценария) Тогда
		Ответ.Тест = ТестированиеЗагрузкаРезультатовТестирования.ТестПоНастройкеСценария(Ответ.НастройкаЗапускаСценария, 
						Проект).Ссылка;
	ИначеЕсли ЗначениеЗаполнено(ПараметрыОшибки.UIDНастройкиСценария) Тогда
		...
		Ответ.Тест =  ТестированиеЗагрузкаРезультатовТестирования.ТестПоUIDНастройкиСценария(Параметры,
						...
						Проект).Ссылка;
	ИначеЕсли ЗначениеЗаполнено(ПараметрыОшибки.ИмяТеста) ИЛИ ЗначениеЗаполнено(ПараметрыОшибки.ВидПроблемы) Тогда 
	...
	Если ЗначениеЗаполнено(Ответ.Тест) Тогда
		РеквизитыТеста = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ответ.Тест, 
						"Ответственный, НастройкаЗапускаСценария, НастройкаЗапускаПроцесса, UIDНастройкиСценария");
...
Функция ПодобратьДанныеВетки(Проект, ИмяВетки)
	...
	Если ЗначениеЗаполнено(Ответ.ВеткаОбнаружения) Тогда
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ответ.ВеткаОбнаружения, "Тип, Ответственный");
...
#Область ПроцедурыРегистрацииОшибок_Формат_1_11
...
Функция СоздатьОписаниеОшибки() Экспорт 
	...
	Ошибка.Вставить("ВерсияФормата", "1.11");
