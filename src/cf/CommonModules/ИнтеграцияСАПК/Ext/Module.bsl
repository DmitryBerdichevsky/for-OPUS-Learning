	...
	Для Каждого ГруппаОшибок Из ГруппыОшибокКЗаписи Цикл
		...
		Для Каждого СтрокаОшибки Из ОшибкиКЗаписи Цикл
			...
			Иначе 
				...
				СтрокаМетаданных = СоздаваемаяОшибка.ИзмененныеМетаданные.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаМетаданных, СтрокаОшибки, "ОбъектМетаданных,ПодчиненныйОбъект");
				СтрокаМетаданных.УточнениеРасположения = СтрокаОшибки.Уточнение;
		...
		СоздаваемаяОшибка.ИзмененныеМетаданные.Сортировать("ОбъектМетаданных,ПодчиненныйОбъект,УточнениеРасположения");
		...
		Если ЕстьНенайденныеМетаданные Тогда
			...
			Если СоздаваемаяОшибка.ИзмененныеМетаданные.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли;
