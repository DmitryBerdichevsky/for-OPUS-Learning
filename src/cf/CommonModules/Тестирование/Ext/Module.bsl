...
//  Строка
//
Функция ПолучитьОтформатированныеТекстыСценариевПроцесса(ДополнительныеПараметры,МассивРолей,Процесс) Экспорт
	...
	ТекстСценария = ПолучитьТекстыСценариевПроцесса(ПараметрыФормированияТекстаСценария,Процесс);
	...
	Возврат ПолучитьОбычныйТекстСценарияИзФорматированногоДокументаСервер(ФД,ДополнительныеПараметры);
КонецФункции	 
...
//  Строка
//
Функция ПолучитьТекстыСценариевПроцесса(ПараметрыФормированияТекстаСценария,Процесс) Экспорт
	ОбъектТекстыСценариев = ПолучитьОбъектнуюМодельТекстыСценариевСервер(Процесс,ПараметрыФормированияТекстаСценария);
...
Функция ФиксироватьИзменения(ДанныеИзмененныхСценариев, ДанныеПодключения, ДанныеФиксацииИзменений) Экспорт
	...
	emailКомуОтправитьЗапросНаСлияние = Неопределено;
	...
	Если ДанныеФиксацииИзменений.СоздатьМерджРеквест Тогда
		...
		МассивСсылок = Новый Массив;
		МассивСсылок.Добавить(ДанныеФиксацииИзменений.КомуОтправить);
		КонтактнаяИнформация = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(
			МассивСсылок,,Справочники.ВидыКонтактнойИнформации.EmailПользователя);
			
		СтрокаКонтактнойИнформации = КонтактнаяИнформация.Найти(ДанныеФиксацииИзменений.КомуОтправить, "Объект");
		Если СтрокаКонтактнойИнформации <> Неопределено Тогда
			emailКомуОтправитьЗапросНаСлияние = СтрокаКонтактнойИнформации.Представление;
		КонецЕсли;	 
		Если НЕ ЗначениеЗаполнено(emailКомуОтправитьЗапросНаСлияние) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Адрес = СтрЗаменить(Адрес, "%ИмяПроекта",
		СтрЗаменить(ДанныеПодключенияПроекта.РедактированиеСценариевВGitИмяПроекта, "/", "%2F"));
	...
	Исключение
		...
		Если Найти(НРег(ОписаниеОшибки), НРег("A file with this name already exists")) > 0 Тогда
			Для Каждого Элем Из СтруктураЗаписи.actions Цикл
				Если НРег(Элем.action) = НРег("create") Тогда
	...
	СтруктураЗаписи.Вставить("id", СтрЗаменить(ДанныеПодключенияПроекта.РедактированиеСценариевВGitИмяПроекта, "/", "%2F"));
	...
	СтруктураЗаписи.Вставить("assignee_id", ИдентификаторПользователяСистемыКонтроляВерсий(
		emailКомуОтправитьЗапросНаСлияние, Соединение, ДанныеПодключения.ТокенПользователя));
...
Функция ПолучитьОбычныйТекстСценарияИзФорматированногоДокументаСервер(ФД,ДополнительныеПараметры)
	Возврат ТестированиеГрафическиеСхемыСервер.ОбычныйТекстСценарияИзФорматированногоДокумента(ФД,ДополнительныеПараметры);
КонецФункции	

Функция ПолучитьОбъектнуюМодельТекстыСценариевСервер(Процесс,ПараметрыФормированияТекстаСценария)
...
Функция ДанныеВеткиРепозитория(Соединение, ДанныеПодключенияПроекта, ТокенПользователя, ИмяВеткиСтрокой)
	...
	Ресурс = СтрЗаменить(Ресурс, "%ИмяВетки", СтрЗаменить(ИмяВеткиСтрокой, "/", "%2F"));
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			СтрШаблон(НСтр("ru = 'Получение данных ветки <%1> из репозитория'"), ИмяВеткиСтрокой),
...
Функция ДвоичныеДанныеФайлаИерархии(ДанныеПодключенияПроекта, ТокенПользователя, Ветка, ИдентификаторКоммита)
	...
	Ресурс = СтрЗаменить(Ресурс, "%ПутьКФайлу",
		СтрЗаменить(ДанныеПодключенияПроекта.КаталогТестовВРепозитории, "/", "%2F") + "%2Fscencache.json");
...
Функция ДанныеФайлаИзРепозитория(Соединение, ДанныеПодключенияПроекта, ТокенПользователя, Ветка, ОтносительныйПутьКФайлу, ИдентификаторКоммита)
	...
	Ресурс = СтрЗаменить(Ресурс, "%ПутьКФайлу", СтрЗаменить(ОтносительныйПутьКФайлу, "/", "%2F"));
